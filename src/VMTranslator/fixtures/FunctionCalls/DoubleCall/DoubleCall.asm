...
(Main.DoubleCall)	// COMMAND #11: function Main.DoubleCall 0
	@SP         
	D=M          // store SP value
	@LCL        
...
(Sys.init)  	// COMMAND #16: function Sys.init 0
	@SP         
	D=M          // store SP value
	@LCL        
	M=D          // store stack address in LCL (no local vars so we're done)
	
	// COMMAND #17: push constant 3
	@3          
	D=A          // store the current address as a value
	...
	// COMMAND #18: call Main.DoubleCall 1
...
(WHILE.VM)  	// COMMAND #19: label WHILE
	
	// COMMAND #20: goto WHILE
	@WHILE.VM   
	0;JMP       
